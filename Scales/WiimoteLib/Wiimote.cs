...
//////////////////////////////////////////////////////////////////////////////////
//** There are some major changes to this file for. Get original version from links above
...
namespace Messure.WiimoteLib
{
    ...
    public class Wiimote : IDisposable
	{
	    ...
	    internal static Task<Wiimote> Find() {
	         return Task.Run(() => {
	            Wiimote mote = null;
	             do {
	                 Thread.Sleep(1000);
                 } while (!TryFindWiimote(ref mote));
        ...
        private static System.Collections.Generic.List<string> mRegisteredWiimoteList = new System.Collections.Generic.List<string>();
		...
		internal static void FindWiimote(WiimoteFoundDelegate wiimoteFound)
		{
				...
				if(HIDImports.SetupDiGetDeviceInterfaceDetail(hDevInfo, ref diData, ref diDetail, size, out size, IntPtr.Zero))
				{
					...
					if(HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
					{
						...
						if(attrib.VendorID == VID && attrib.ProductID == PID)
						{
                            ...
                            if (mRegisteredWiimoteList.Exists(item => item.Equals(diDetail.DevicePath)) == true)
                            {
                                // already registered WiiMote
                                Debug.WriteLine("already registered!");
                                found = false;
                            }
                            else
                            {
                                mRegisteredWiimoteList.Add(diDetail.DevicePath);
            ...
            if (!found)
            {
                if (mRegisteredWiimoteList.Count == 0)
                throw new WiimoteNotFoundException("No Wiimotes found in HID device list.");
            }
		}
        internal static bool TryFindWiimote(ref Wiimote wiimote)
        {
            int index = 0;
            bool found = false;
            Guid guid;
                ...
                if (HIDImports.SetupDiGetDeviceInterfaceDetail(hDevInfo, ref diData, ref diDetail, size, out size, IntPtr.Zero))
                {
                    ...
                    if (HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
                    {
                        ...
                        if (attrib.VendorID == VID && attrib.ProductID == PID)
                        {
                            ...
                            wiimote = new Wiimote(diDetail.DevicePath);
		...
		private void OpenWiimoteDeviceHandle(string devicePath)
		{
			...
			if(HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
			{
				...
				if(attrib.VendorID == VID && attrib.ProductID == PID)
				{
                    ...
                    mRegisteredWiimoteList.Add(devicePath);
		...
		public void Disconnect()
		{
            ...
            bIsConnected = false;
            mRegisteredWiimoteList.Remove(mDevicePath);
		}
		...
		{
			// if the stream is valid and ready9
